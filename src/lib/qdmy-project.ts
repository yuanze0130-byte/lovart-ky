...
function nodeToElement(rawNode: unknown, groupId?: string): CanvasElement | null {
  ...
  const element: CanvasElement = {
    ...
    prompt,
    aiModel: firstString(node.aiModel, settings.aiModel, (type === 'ai-agent' || type === 'ai-text') ? settings.selectedChatModel : undefined),
    aiInstruction: firstString(node.aiInstruction, settings.aiInstruction, (type === 'ai-agent' || type === 'ai-text') ? prompt : undefined),
    aiSystemPrompt: firstString(node.aiSystemPrompt, settings.aiSystemPrompt, type === 'ai-agent' ? settings.systemPrompt : undefined),
    speechModel: type === 'speech-generator' ? firstString(node.speechModel, settings.speechModel) : undefined,
    speechVoiceId: type === 'speech-generator' ? firstString(node.speechVoiceId, settings.speechVoiceId) : undefined,
    speechSpeed: type === 'speech-generator' ? firstNumber(node.speechSpeed, settings.speechSpeed) : undefined,
    speechPitch: type === 'speech-generator' ? firstNumber(node.speechPitch, settings.speechPitch) : undefined,
    speechAudioFormat: type === 'speech-generator' && ['mp3', 'wav', 'flac'].includes(firstString(node.speechAudioFormat, settings.speechAudioFormat) || '')
      ? firstString(node.speechAudioFormat, settings.speechAudioFormat) as CanvasElement['speechAudioFormat'] : undefined,
    musicMode: type === 'music-generator' && firstString(node.musicMode, settings.musicMode) === 'custom' ? 'custom' : type === 'music-generator' ? 'inspiration' : undefined,
    musicTitle: type === 'music-generator' ? firstString(node.musicTitle, settings.musicTitle, node.title) : undefined,
    musicStyle: type === 'music-generator' ? firstString(node.musicStyle, settings.musicStyle, settings.tags) : undefined,
    musicInstrumental: type === 'music-generator' && typeof (node.musicInstrumental ?? settings.musicInstrumental ?? settings.make_instrumental) === 'boolean'
      ? Boolean(node.musicInstrumental ?? settings.musicInstrumental ?? settings.make_instrumental) : undefined,
    musicVersion: type === 'music-generator' ? firstString(node.musicVersion, settings.musicVersion, settings.mv, model) : undefined,
    assetReferenceIds: asArray(node.assetReferenceIds ?? settings.assetReferenceIds).filter((value): value is string => typeof value === 'string'),
    ...
    content: type === 'text'
      ...
      : type === 'ai-agent' ? firstString(content, settings.outputContent) : content,
  ...
  const nodes = input.elements.filter((element) => element.type !== 'connector').map((element) => {
    ...
    return {
      ...
      settings: {
        ...
        musicStyle: element.musicStyle,
        musicInstrumental: element.musicInstrumental,
        musicVersion: element.musicVersion,
        mv: element.musicVersion,
        tags: element.musicStyle,
        make_instrumental: element.musicInstrumental,
