...
同步工具会在同一事务中更新价格配置、分组倍率、路由渠道、渠道模型列表、`abilities` 模型可用性索引、模型中文描述和当前令牌分组，并保留非 Comfly 的已有定价。
...
模型广场的描述也由同一个同步任务维护，当前可用的 1019 个模型均已有中文描述：

- Comfly 返回中文描述时，优先同步其原始中文内容；
- Comfly 未提供中文描述或只提供英文时，按模型名称、分类及输入输出模态生成简洁的中文说明；
- 已人工填写的中文描述不会被无中文来源的通用说明覆盖；
- 新增模型会自动写入 `models` 元信息表，并按名称规则关联本地供应商；
- 描述同步不修改模型标识、价格、渠道密钥或用户令牌权限。

同步完成后，脚本输出中的 `metadata_drift_total` 应为 `0`。前台模型广场有约一分钟缓存，后台同步成功后可能需要稍等并刷新页面。
...
4. 模型中文描述已全量覆盖，但上游缺少原始说明的模型使用自动生成的通用描述；重要或热门模型可继续在 New API 后台人工润色。
